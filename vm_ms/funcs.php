...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://online.moysklad.ru/api/remap/1.2/entity/customerorder/metadata/',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_USERPWD=> "api@manager245:api1111", 
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
...
function getIdOrder($name) {
  $curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://online.moysklad.ru/api/remap/1.2/entity/customerorder?search='.$name,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_USERPWD=> "api@manager245:api1111", 
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
...
function putStatus($id,$ordStatus) {
$curl = curl_init();

$postData = array();
$postData['state']['meta'] = $ordStatus;
$postData = json_encode($postData, 256); // 256 - для кодировки русского языка
// dd($postData);exit;
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://online.moysklad.ru/api/remap/1.3/entity/customerorder/'.$id,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_USERPWD=> "api@manager245:api1111", 
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
...
function getOrderMs() {
  ...
  $s = date("Y-m-d");
  // $today = date("Y-m-d",strtotime(date("H:i:s")." - 60 minutes"));
  // $s = date("Y-m-d");
  $data = $s." ".$today;
  $today = date("Y-m-d",strtotime(date("Y-m-d")." - 1 day"));
  // dd($today);
  curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://online.moysklad.ru/api/remap/1.2/entity/customerorder?filter=moment%3E'.$today ,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_USERPWD=> "api@manager245:api1111", 
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
...
function getCurlData($url, $account, $password)   {
  ...
  // file_put_contents('hook.txt',date('H:i:s').' state_func '.$res."\n",FILE_APPEND);
