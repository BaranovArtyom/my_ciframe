...
namespace Brick\PhoneNumber\Shopify;
    ...
    foreach ( $orders->items as $orderEcw ) {
        ...
        if (!file_exists($orders_dir . $orderEcw->id)) {
            ...
            //    dd($item->discounts);die();
                ...
                foreach( $productsShopify->products as $product ) {
                    foreach( $product->variants as $variant ) {
                        ...
                        // }
                        // dd($shippingAddress['phone']);die();
                        // echo $shippingAddress['phone'];
                        $shippingAddress['phone'] = PhoneNumber::parse($shippingAddress['phone'], $shippingAddress['country_code']);
                        

                        $shippingAddress['phone'] = $shippingAddress['phone']->format(PhoneNumberFormat::E164);
                        // $shippingAddress['phone'] = substr($shippingAddress['phone'], 0, -2);

                        // echo $shippingAddress['phone'];die();
                        // // dd($number->countryCode); die();
                       
                        // var_dump($shippingAddress['phone']);die();
            ...
            if ( !isset($createOrders->errors)) {
                ...
                unlink(__DIR__."/sync_orders/$orderEcw->id");
            }
            // dd($updateOrdersEcwid);
        }else {
